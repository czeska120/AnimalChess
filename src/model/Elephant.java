package model;

import java.awt.*;
import java.util.LinkedList;
/**
 * A subclass of Piece.
 *
 * @author ALABANZA, Chelsea
 * @author SILVESTRE, Franczeska
 */
public class Elephant extends Piece
{
    /**
     * Elephant constructor.
     *
     * @param n - name of Elephant
     * @param posX - x coordinate
     * @param posY - y coordinate
     * @param p - determines to belong to which player
     * @param ps - list to be added into
     * @param r - rank of Elephant
     * @param color - Color of JButton
     */
    public Elephant(String n, int posX, int posY, boolean p, LinkedList<Piece> ps, int r, Color color)
    {
        super(n,posX,posY,p,ps, r, color );
        this.bSwim = false;
        this.bJump = false;
    }

    /**
     * called whenever Elephant is not inside of Trap.
     */
    public void revertRank()
    {
        this.bTrapped = false;
        this.rank = 8;
    }

    /**
     * Overridden capture to satisfy Elephant conditions.
     *
     * @param otherP - Piece to be captured
     * @return true if valid, false otherwise
     */
    @Override
    public boolean captures(Piece otherP)
    {
        if(this.isP1() != otherP.isP1()) // First check if opponent's piece
        {
            if( otherP instanceof Mouse && !(otherP.bTrapped))
            {
                return false;
            }

            else if(getRank() >= otherP.getRank()) // Follows ranking to capture
            {
                kill(otherP);
                return true;
            }
            else
            {
                return false;
            }
        }
        else
        {
            return false;
        }
    }
}
